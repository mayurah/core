...
class RootTest extends \Test\TestCase {
	...
	/**
	 * We expect that calling get('/bar/foo') twice will only invoke getFileInfo once (will get it from cache)
	 */
	public function testGetCached() {
		...
		// Call first time and will use getFileInfo
		$node = $root->get('/bar/foo');
		$this->assertEquals(10, $node->getId());
		$this->assertInstanceOf('\OC\Files\Node\File', $node);

		// Call 2nd time and will get FileInfo from cache
		$node = $root->get('/bar/foo');
		$this->assertEquals(10, $node->getId());
		$this->assertInstanceOf('\OC\Files\Node\File', $node);
	}
